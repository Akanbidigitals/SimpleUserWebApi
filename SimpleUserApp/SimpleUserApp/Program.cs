...
using SimpleUserApp.DataAccess.Repository;
...
builder.Services.AddDbContext<SimpleUserContext>(options =>
{
    options.UseSqlServer(builder.Configuration.GetConnectionString("SimpleUser"));
});

builder.Services.AddScoped<ISimpleUser, SimpleUserRepository>();
